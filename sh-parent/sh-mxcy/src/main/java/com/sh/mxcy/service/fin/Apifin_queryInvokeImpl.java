 ...
 * @description 成品仓库-查询
 */
@Service("Apifin_queryInvokeImpl")
public class Apifin_queryInvokeImpl implements ServiceHandlerInvoke {
    ...
    public RetJSONObject handler(JSONObject inParamJO) throws Exception {
        ...
        if (ret.isSuccess()) {
            //参数复制
            HandlePubDataUtil.paramCopy(inParamJO, inDBJSONObject, "page_index", "page_size", "plan_num", "figure_no", "cre_time_start", "cre_time_end");
            //处理分页参数
            HandlePubDataUtil.limitParmHandle(inDBJSONObject);
            int total = FinDao.query_total(inDBJSONObject);
            List<OutDBJSONObject> outDBJSONObject = FinDao.query(inDBJSONObject);
