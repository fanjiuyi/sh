 ...
 * @description 原材料-修改
 */
@Service("Apimaterial_editInvokeImpl")
public class Apimaterial_editInvokeImpl implements ServiceHandlerInvoke {
    ...
    public RetJSONObject handler(JSONObject inParamJO) throws Exception {
        ...
        if (ret.isSuccess()) {
            //参数复制
            HandlePubDataUtil.paramCopy(inParamJO, inDBJSONObject, "id", "name", "spec", "pro_unit", "mea_unit", "pro_num", "type_code", "remarks","sur_num");

            MaterialDao.update(inDBJSONObject);
