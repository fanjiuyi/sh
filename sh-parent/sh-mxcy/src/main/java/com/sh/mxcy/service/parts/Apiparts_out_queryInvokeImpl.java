 ...
 * @description 配件-出库查询
 */
@Service("Apiparts_out_queryInvokeImpl")
public class Apiparts_out_queryInvokeImpl implements ServiceHandlerInvoke {
    ...
    public RetJSONObject handler(JSONObject inParamJO) throws Exception {
        ...
        if (ret.isSuccess()) {
            //参数复制
            HandlePubDataUtil.paramCopy(inParamJO, inDBJSONObject, "page_index", "page_size", "id", "name", "user", "cre_time_start", "cre_time_end");
            //处理分页参数
            HandlePubDataUtil.limitParmHandle(inDBJSONObject);
            int total = PartsDao.out_query_total(inDBJSONObject);
            outDBJSONObject = PartsDao.out_query(inDBJSONObject);
