 ...
 * @description 委外-修改
 */
@Service("Apientr_editInvokeImpl")
public class Apientr_editInvokeImpl implements ServiceHandlerInvoke {
    ...
    public RetJSONObject handler(JSONObject inParamJO) throws Exception {
        ...
        if (ret.isSuccess()) {
            //参数复制
            HandlePubDataUtil.paramCopy(inParamJO, inDBJSONObject, "id", "name", "figure_no", "entr_unit", "num", "type_code", "pro_plan_id", "entr_name", "start_time", "end_time", "remarks");

            EntrDao.update(inDBJSONObject);
